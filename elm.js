...
Elm.Main.make = function (_elm) {
   ...
   square) {
      return function () {
         var mouth = $List.head(snake.parts);
         return function () {
            switch (mouth.ctor)
            {case "Just":
   ...
   var stealApple = F2(function (snake,
   apple) {
      return _U.replace([["owner"
                         ,$Maybe.Just(snake)]],
      apple);
   });
   var gettingEaten = F3(function (snake,
   snake2,
   apple) {
      return A2(collision,
      snake,
      apple.square) ? A2(stealApple,
      snake,
      apple) : A2(collision,
      snake2,
      apple.square) ? A2(stealApple,
   ...
   0,
   255);
   var snakeColor = A3($Color.rgb,
   255,
   0,
   255);
   var boardColor = A3($Color.rgb,
   0,
   0,
   0);
   ...
   var defaultParts = function (length) {
      ...
      {ctor: "_Tuple2"
      ,_0: length - 1 - startingLength
      ,_1: 0},
      defaultParts(length - 1));
   };
   var defaultSnake = F2(function (dir,
   color) {
      return {_: {}
             ,chomp: Air
             ,color: color
   ...
   var renderApples = function (apples) {
      return A2($List.map,
      function (a) {
         return A2(placeSegment,
         ownerColor(a),
         a.square);
      },
      apples);
   };
   var renderSquares = F2(function (squares,
   color) {
      return A2($List.map,
      placeSegment(color),
      squares);
   ...
   var boardSize = 700;
   var range = (boardSize / (scale * 2) | 0) - 1;
   var placeApple = function (apple) {
      return function () {
   ...
   var defaultApples = function (length) {
      return _U.eq(length,
      0) ? _L.fromArray([]) : A2($List._op["::"],
      placeApple(appleConstructor(startTime * length)),
      defaultApples(length - 1));
   };
   var defaultGame = {_: {}
                     ,apples: defaultApples(startingApples)
                     ,boardSize: boardSize
                     ,snake: A2(defaultSnake,
                     -1,
                     snakeColor)
   ...
   _v28) {
      return function () {
         return function () {
            ...
            outsideBorder,
            snake.parts) ? Border : Air;
         }();
      }();
   });
   var updateSnake = F2(function (snake,
   gameState) {
      return function () {
         var snakeState = A2(checkSnakeStatus,
         snake,
         gameState);
         return function () {
            switch (snakeState.ctor)
            {case "Air":
               return movingSnake(snake);
               case "Border": return snake;
               case "Food":
